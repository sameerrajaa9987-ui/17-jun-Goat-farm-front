...
// RN's JS engine sometimes lacks `atob` (used by base64 JWT decoding below).
const runtimeGlobal = globalThis as unknown as { atob?: typeof decode };
if (!runtimeGlobal.atob) {
  runtimeGlobal.atob = decode;
}

export type Role = "owner" | "manager" | "worker" | "client" | "vet";
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      logout: async () => {
        await secureStorage.removeItem("goat-auth-storage");
        set({
          user: null,
          token: null,
          refreshToken: null,
